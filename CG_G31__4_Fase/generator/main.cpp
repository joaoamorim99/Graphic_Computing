...
/**/
	for (i = 0; i < slices; i++) {
		for(j = 0; j < stacks; j++){
			...
			//Base de baixo
			MyFile << cos(angle - (beta * j))*cos(alpha * i) << " " << sin(angle - (beta * j)) << " " << cos(angle - (beta * j))*sin(alpha * i) << endl;
			MyFile <<  cos(angle - (beta * (j+1))) * cos(alpha * i) << " " << sin(angle - (beta * (j + 1))) << " " << cos(angle - (beta * (j + 1))) * sin(alpha * i)<< endl;
			MyFile <<  cos(angle - (beta * (j+1))) * cos(alpha * (i+1)) << " " << radius * sin(angle - (beta * (j + 1))) << " " << radius * cos(angle - (beta * (j + 1))) * sin(alpha * (i+1))<< endl;
			MyFile <<  cos(angle - (beta * j)) * cos(alpha * (i+1)) << " " << sin(angle - (beta * j)) << " " << cos(angle - (beta * j)) * sin(alpha*(i+1))<< endl;
		
		}
	}

	//Escreve nº de coordenadas de textura
	MyFile << tt_vertices << endl;


	//Escreve coordenada de textura
	for (i = 0; i < slices; i++) {
		for(j = 0; j < stacks; j++){
			//x = radius * sin(beta * i);

			//Base de baixo
			MyFile <<  ((2*M_PI)-alpha*i) / (2*M_PI) << " " << (alpha*j) / (2*M_PI) << endl;
			MyFile <<  ((2*M_PI)-alpha*i) / (2*M_PI) << " " << (alpha*(j+1)) / (2*M_PI) << endl;
			MyFile <<  ((2*M_PI)- alpha*(i+1)) / (2*M_PI) << " " << (alpha*(j+1)) / (2*M_PI) << endl;
			MyFile <<  ((2*M_PI)- alpha*(i+1)) / (2*M_PI) << " " << (alpha*j) / (2*M_PI) << endl;
			//MyFile << (slices - j) / slices << " " << (stacks - i) / stacks << endl;
...
void plane(float length,float divisions,string namefile) {
	...
	vector<int> indices; //os índices são guardados neste vetor
	int size = 0;
	...
	for(float i = -divisions / 2; i * divX < x / 2; i++) {
		for(float j = -divisions / 2; j * divZ < z / 2; j++ ) {
			...
			Myfile << 0.0f <<" "<< 0.0f << endl;
			Myfile << 1.0f <<" "<< 1.0f << endl;
			Myfile << 1.0f <<" "<< 0.0f << endl;
			Myfile << 0.0f <<" "<< 1.0f << endl;
...
void cone(float radius,float height, int slices, int stacks,string namefile) {
	...
	//Escreve as normais no ficheiro
	for (i = 0; i < slices; i++) {
		//Base de baixo
		MyFile << 0.0f <<" "<< -1.0f <<" "<< 0.0f << endl;
		MyFile<< 0.0f <<" "<< -1.0f <<" "<< 0.0f << endl;
		...
		MyFile<< 0.0f <<" "<< 1.0f <<" "<< 0.0f << endl;
		MyFile<< cos(alpha * (i + 1)) <<" "<< 0.0f <<" "<< sin(alpha * (i + 1)) << endl;
		MyFile<< cos(alpha * i) <<" "<< 0.0f <<" "<< sin(alpha * i) << endl;
		MyFile<< 0.0f <<" "<< 1.0f <<" "<< 0.0f << endl;
...
void handler(int argc, char* argv[]){
    ...
    string c = "cone";
    string comm = argv[1];
    string namefile = "C:/Users/jonny/OneDrive/Ambiente de Trabalho/Apresentacao/CG_G31__4_Fase/SASAS/engine/testes/";
